...
public:
   ...
   */
  void swapBuffers(QRect region, EPContentType epct, EPScreenMode type,
   ...
   * );
   */
  void swapBuffers(const QRegion &region, const EPContentMap &contentMap,
   ...
   */
  inline void update(QRect rect, int waveform, PixelMode pixelMode,
                     QFlags<EPFramebuffer::UpdateFlag> flags) {
    static const auto fn =
        []() -> std::function<void(void *, QRect, int, PixelMode,
                                   QFlags<EPFramebuffer::UpdateFlag>)> {
#ifdef __arm__
      auto sendTConUpdate = reinterpret_cast<void (*)(
          void *, const QRect &, int, QFlags<EPFramebuffer::UpdateFlag>)>(
          dlsym(RTLD_DEFAULT, "_ZN17EPFramebufferTcon14sendTConUpdateERK5QRecti"
                              "6QFlagsIN13EPFramebu"
                              "ffer10UpdateFlagEE"));
      if (sendTConUpdate != nullptr) {
        return [sendTConUpdate](void *this_ptr, QRect rect, int waveform,
                                PixelMode pixelMode,
                                QFlags<EPFramebuffer::UpdateFlag> flags) {
          Q_UNUSED(pixelMode);
...
#endif
      return reinterpret_cast<void (*)(void *, QRect, int, PixelMode,
                                       QFlags<EPFramebuffer::UpdateFlag>)>(
          dlsym(RTLD_DEFAULT,
                "_ZN19EPFramebufferSwtcon6updateE5QRecti9PixelModei"));
    }();
    if (fn == nullptr) {
      qFatal("Unable to find EPFramebuffer::update()"
#ifdef __arm__
...
private:
  char OPAQUE_B[EPFR_OFFSET_RENDERTARGET - EPFR_OFFSET_FRAMEBUFFER -
                sizeof(QImage)];
...
private:
  char OPAQUE_C[EPFR_OFFSET_PREVIOUSBUFFER - EPFR_OFFSET_RENDERTARGET -
                sizeof(QImage *)];
